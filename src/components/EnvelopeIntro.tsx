...
export const EnvelopeIntro = ({
  ...
  const handleTriggerFadeIntoSite = useCallback(() => {
    ...
    // Pause video to ensure it doesn't replay
    if (videoRef.current) {
      videoRef.current.pause();
    }

    // Smoothly dissolve the intro and reveal the full wedding invitation
    setTimeout(() => {
      onOpenInvitation();
    }, 800);
  }, [onOpenInvitation]);
  ...
  useEffect(() => {
    ...
    // Explicitly ensure loop is disabled
    video.loop = false;
    ...
    const attemptPlay = async () => {
      try {
        ...
        // If unmuted autoplay is blocked by browser policy, ensure muted and play
        video.muted = true;
        try {
          await video.play();
        } catch {
          // If still blocked, user tap listener handles it
        }
      }
    };
  ...
  const handleTimeUpdate = () => {
    ...
    // Detect when video reaches near the end (duration is ~2.7s)
